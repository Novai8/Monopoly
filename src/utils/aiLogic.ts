...
function difficultyMultiplier(difficulty: BotDifficulty): number { return { easy: 0.78, normal: 1, hard: 1.16, expert: 1.28 }[difficulty]; }
function groupProgressValue(player: Player, tile: BoardTile, tiles: BoardTile[], ownership: OwnershipMap): number {
  ...
  return (completion + (opponents > 0 ? 0.45 : 0)) * 100;
}
...
function opponentExposure(player: Player, tiles: BoardTile[], ownership: OwnershipMap, players: Player[]): number {
  return players.filter((candidate) => candidate.id !== player.id && !candidate.bankrupt).reduce((total, opponent) => {
...
export function calculatePropertyValue(player: Player, tile: BoardTile, tiles: BoardTile[], ownership: OwnershipMap, players: Player[] = []): number {
  ...
  const blocking = groupProgressValue(player, tile, tiles, ownership) * personality.blocking * 0.35;
  const exposure = opponentExposure(player, tiles, ownership, players);
  return Math.round((tile.cost + group + rent + development + blocking + exposure) * difficulty);
}
export function getAISafeCashReserve(player: Player, tiles: BoardTile[], ownership: OwnershipMap): number {
  const personality = PERSONALITY_WEIGHTS[personalityOf(player)];
  const base = player.difficulty === 'easy' ? 80 : player.difficulty === 'hard' ? 220 : player.difficulty === 'expert' ? 300 : 140;
...
export function shouldAIBidOnAuction(aiPlayer: Player, tile: BoardTile, currentBid: number, tiles: BoardTile[], ownership: OwnershipMap, players: Player[] = []): { shouldBid: boolean; maxBid: number } {
  ...
  if (tile.group && getOwnedCountInGroup(tile.group, aiPlayer.id, tiles, ownership) === tiles.filter((candidate) => candidate.group === tile.group).length - 1) maxBid = Math.min(aiPlayer.balance - Math.max(25, Math.round(reserve * 0.65)), Math.round(maxBid * (1.25 + personality.group * 0.18)));
  maxBid = Math.max(0, maxBid);
  const nextBid = currentBid + 10;
  const deliberatePass = difficulty === 'easy' && nextBid > tile.cost * 0.95;
  return { shouldBid: !deliberatePass && nextBid <= maxBid && aiPlayer.balance >= nextBid, maxBid };
}
...
export function evaluateAITrade(offer: TradeOffer, aiPlayer: Player, tiles: BoardTile[], ownership: OwnershipMap): { accept: boolean; reason: string } {
  let offeredVal = offer.offeredMoney; let requestedVal = offer.requestedMoney;
  for (const id of offer.offeredTileIds) { const tile = tiles.find((candidate) => candidate.id === id); if (tile) offeredVal += calculatePropertyValue(aiPlayer, tile, tiles, ownership); }
  for (const id of offer.requestedTileIds) { const tile = tiles.find((candidate) => candidate.id === id); if (tile) requestedVal += calculatePropertyValue(aiPlayer, tile, tiles, ownership); }
  const ratio = aiPlayer.difficulty === 'expert' ? 1.12 : aiPlayer.difficulty === 'hard' ? 1.04 : aiPlayer.difficulty === 'easy' ? 0.9 : 0.98;
  const reserve = getAISafeCashReserve(aiPlayer, tiles, ownership);
  return offeredVal >= requestedVal * ratio && aiPlayer.balance + offer.offeredMoney - offer.requestedMoney >= reserve
...
export function calculateAIBid(aiPlayer: Player, tile: BoardTile, tiles: BoardTile[], ownership: OwnershipMap): number { return shouldAIBidOnAuction(aiPlayer, tile, 0, tiles, ownership).maxBid; }
