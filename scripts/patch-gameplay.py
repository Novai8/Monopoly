...
function getBotAction(room: MultiplayerRoom): { botId: string; action: ClientAction } | null {
  ...
  if (gs.gamePhase === 'auction' && gs.auction?.active && gs.auction.currentBidderId) {
    ...
    return wantsProperty && bot.balance >= nextBid
      ? { botId: bot.id, action: { type: 'PLACE_BID', amount: nextBid } }
      : { botId: bot.id, action: { type: 'PASS_AUCTION' } };
  }
  ...
  switch (gs.gamePhase) {
    ...
    case 'action-required': {
      if (gs.canBuyProperty) {
        ...
        return tile && shouldAIBuyProperty(bot, tile, tiles, gs.ownership)
          ? { botId: bot.id, action: { type: 'BUY_PROPERTY', tileId: tile.id } }
          : { botId: bot.id, action: { type: 'DECLINE_PROPERTY', tileId: bot.position } };
      }
...
s = s.replace("const binding = socketToPlayer.get(ws);", "const binding = getActionBinding(ws, actorId);")
...
bot_cases = r'''    case 'ADD_BOT': {
      ...
      const bot: Player = {
        id: `bot-${generateId()}`, name: uniqueName, isAI: true, isBot: true, color,
        character: botCharacters[botIndex % botCharacters.length], balance: room.settings.startingMoney || 1500,
        position: 0, inDetention: false, detentionTurns: 0, detentionPasses: 0, bankrupt: false,
        voiceState: 'quiet', isHost: false, ready: true, difficulty: action.difficulty || room.settings.botDifficulty || 'normal',
      };
...
function resolveLandingOutcome(room: MultiplayerRoom, playerId: string, diceTotal: number, allowExtraRoll: boolean): void {
  ...
  if (landing.type === 'unowned') {
    gs.gamePhase = 'action-required'; gs.canBuyProperty = true;
    ...
    if (player.balance >= landing.amount) {
      player.balance -= landing.amount; if (owner) owner.balance += landing.amount;
      gs.gamePhase = allowExtraRoll ? 'ready-to-roll' : 'turn-end';
    } else {
      const bankruptcy = GameEngine.handleBankruptcy(player, owner?.id || null, gs.players, gs.ownership);
      gs.players = bankruptcy.updatedPlayers; gs.ownership = bankruptcy.updatedOwnership; gs.winner = bankruptcy.winner;
      gs.logs.unshift({ id: generateId(), text: bankruptcy.message, type: 'bankruptcy', timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) });
      gs.gamePhase = bankruptcy.winner ? 'game-over' : 'turn-end';
    }
  } else if (landing.type === 'tax' && landing.amount) {
    if (player.balance >= landing.amount) {
      player.balance -= landing.amount; gs.gamePhase = allowExtraRoll ? 'ready-to-roll' : 'turn-end';
    } else {
      const bankruptcy = GameEngine.handleBankruptcy(player, null, gs.players, gs.ownership);
      gs.players = bankruptcy.updatedPlayers; gs.ownership = bankruptcy.updatedOwnership; gs.winner = bankruptcy.winner;
      gs.gamePhase = bankruptcy.winner ? 'game-over' : 'turn-end';
    }
  } else if (landing.type === 'card' && landing.card) {
    gs.drawnCard = landing.card; gs.gamePhase = 'card-choice';
  } else if (landing.type === 'detention') {
    player.inDetention = true; player.detentionTurns = 0;
    const jail = tiles.find((candidate) => candidate.type === 'detention'); if (jail) player.position = jail.id;
...
replacement = "      resolveLandingOutcome(room, activePlayer.id, sum, isDoubles && !activePlayer.inDetention);\n\n      broadcastToRoom(room.code, { type: 'STATE_UPDATE', gameState: gs });\n      break;\n    }\n\n    case 'BUY_PROPERTY':"
...
# Multiplayer client: expose existing ClientAction bot commands to the lobby.
